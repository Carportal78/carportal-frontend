...
import { selectBrandAtom, selectCityAtom } from "../atoms/categoriesAtoms";
import statesCitiesList from '../../../public/jsondata/state-and-city.json';
import BreadCrumb from "../listing/listing-single/BreadCrumb";


export default function DealersPageDescription({ carModelDetails, carVariantsList, carBrandsList, carBrand, cityData, brandData }) {
    // State for the UI
    ...
    function handleSearchDealer() {
        setBrandData(selectedBrandGroup?.value);
        setCityData(selectedCity?.value);
        localStorage.setItem('dealer-type', JSON.stringify({ brand: selectedBrandGroup?.value, city: selectedCity?.value }))
        router.push(`/dealers/list/${selectedBrandGroup?.label}/${selectedCity.label}`);
    }

    // Populate brand options
    useEffect(() => {
        if (brandid) {
            const selectedBrand = carBrandsList.find(brand => brand._id === brandData);
            if (selectedBrand) {
        ...
        <div className="row mb10">
            ...
            <div className="p20 bdr_none pl0 pr0">
                <div className="wrapper">
                    {!carBrand && <h4>Search Car Dealers</h4> }
